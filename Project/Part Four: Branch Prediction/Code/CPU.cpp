...
void CPU::decode() {
    ...
    opcode = (instr >> 0x1A);          // Instruction Decode,based of chart
    rs = (instr >> 0x15) & 0x1F;         
    rt = (instr >> 0x10) & 0x1F;        
    rd = (instr >> 0xB) & 0x1F;          
    ...
    switch(opcode) {
        ...
        case 0x04: {
                ...
                uint32_t temp = pc + (simm << 2);
                uint32_t predictedTarget;
                bool misPred, predictedValue, taken;
                predictedValue = bpred.predict(pc, temp);
            
                if(predictedValue){
                    predictedTarget = temp;
                }
                else{
                ...
                if (regFile[rs] == regFile[rt]){
                    misPred = bpred.isMispredict(predictedValue, predictedTarget, true, temp);
                    taken = true;
                    pc = pc + (simm << 2);
                    stats.countTaken();
                }
                else if (regFile[rs] != regFile[rt]){
                    misPred = bpred.isMispredict(predictedValue, predictedTarget, false, pc);
                    taken = false;
                }
                if (misPred){
                    bpred.update(pc, taken, temp);
                    stats.flush(2);
                }
                stats.countBranch();
                break;
        ...
        case 0x05: {
                ...
                if (regFile[rs] != regFile[rt]){
                    //Update isMispredict(bool predTaken, uint32_t predTarget, bool taken, uint32_t target)
                    misPred = bpred.isMispredict(predictedValue, predictedTarget, true, temp);
                    taken = true;
                    pc = pc + (simm << 2);
                    stats.countTaken();
...
void CPU::printFinalStats() {
    ...
    cout << "CPI: "<< fixed << setprecision(2) << ( (float)stats.getCycles() / (float)instructions)<< endl;
